...
public class ScmPurchaseBidLinesVORowImpl extends ViewRowImpl {
    ...
    public void remove() {
        // TODO Implement this method
        if (getRemarks()!=null && getRemarks().equals("!!!DELETING!!!")) {
            super.remove();
            return;
       }
        oracle.jbo.server.RowQualifier rowQualifer = new RowQualifier((ViewObjectImpl)this.getViewObject());
        rowQualifer.setWhereClause("ItemId= "+getItemId()+" AND BidLinesSno!='"+getBidLinesSno());
        Row r[]=((ViewObjectImpl) this.getViewObject()).getFilteredRows(rowQualifer);
        System.out.println(r.length);
        Integer noofrows=r.length;
                if (noofrows>0) {
                    for (Row deleterow:r) 
            {
                deleterow.setAttribute("Remarks", "!!!DELETING!!!");
                deleterow.remove();
           }
